...
def run():
    ...
    sql = "select id as project_id, goal, amount_pledged, backer_count, category, start_date, currency from project  where currency is not Null "
    ...
    for category, category_df in df.groupby(['category']):
        with open('/usr/share/nginx/html/crowdfunding/my/scott/kickstarter/goal_vs_actual/analysis_output/{}.json'.format(category), 'w') as f:
            json.dump(category_df.to_dict(orient='records'), f)
